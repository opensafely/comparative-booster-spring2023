...
BNT162b2 booster recipients were matched with Moderna booster recipients using two different matching strategies. one simpler strategy that will identify more matches but the risk of greater imbalance between treatment groups, and another more complex strategy that will identify fewer matches but with less imbalance. 

#### Matching strategy A

Booster recipients were matched on the following characteristics: date of booster dose; primary course vaccine brand; date of second vaccine dose, within 7 days; NHS region (East of England, Midlands, London, North East and Yorkshire, North West, South East, South West); clinical risk groups used for prioritisation (clinically extremely vulnerable, clinically at-risk, neither); age, within 3 years and age groups used by JCVI for prioritisation; English Index of Multiple Deprivation, grouped by quintile.

#### Matching strategy B

Booster recipients were matched on the following characteristics: date of booster dose; primary course vaccine brand; date of second vaccine dose, within 7 days; Sustainability and Transformation Partnership (STP) a geographical grouping of NHS and local authorities; evidence of prior SARS-CoV-2 infection (positive SARS-CoV-2 test, "probable" infection documented in primary care, or COVID-19 hospital admission); clinical risk groups used for prioritisation (clinically extremely vulnerable, clinically at-risk, neither); age, within 3 years; and age groups used by JCVI for prioritisation; English Index of Multiple Deprivation, grouped by quintile; sex; and morbidity count (0, 1, 2 or more).
...
95% confidence limits for the KM estimates were derived from the complementary log-log transformation. Confidence limits for the risk difference were derived from the sum of squares of the treatment-specific KM standard errors, derived from Greenwood’s formula. Confidence limits for the survival ratio were based on the standard error of the cumulative hazard. Confidence limits for the risk ratio were derived using the Delta method.
...
We also estimated comparative effectiveness separately in the following subgroups: primary vaccine course; age (18-64 or $\ge$ 65 years); clinically extremely .vulnerable or not; evidence of prior SARS-CoV-2 infection or not; dominant SARS-CoV-2 variant era (Delta or Omicron).
...
In this study, we round counts up to the nearest multiple of 7. Kaplan-Meier survival estimates are rounded upwards by deferring event-times until there are at least 7 events, so that each KM “step” is based on at least 7 events. All other quantities will be derived from these rounded KM estimates. 
...
km_estimates <- read_csv(fs::path(output_dir_os, matchset, "km_estimates.csv")) %>% 
  mutate(
...
contrasts_daily <- read_csv(fs::path(output_dir_os, matchset, "contrasts_daily.csv")) %>% 
  mutate(
    outcome_descr = fct_recoderelevel(as.character(outcome),  recoder$outcome),
    subgroup_descr = fct_recoderelevel(subgroup,  recoder$subgroup),
    subgroup_level_descr = replace_na(subgroup_level_descr, "")
  )
contrasts_overall <- read_csv(fs::path(output_dir_os, matchset, "contrasts_overall.csv")) %>% 
  mutate(
    outcome_descr = fct_recoderelevel(as.character(outcome),  recoder$outcome),
    subgroup_descr = fct_recoderelevel(subgroup,  recoder$subgroup),
    subgroup_level_descr = replace_na(subgroup_level_descr, "")
...
For the simpler matching criteria, `r label_number(1, big.mark=",")(filter(match_flowchart, crit=="c7", vax3_type=="pfizer") %>% pull(n))` were successfully matched in each group, `r label_percent(0.1)(filter(match_flowchart, crit=="c7", vax3_type=="pfizer") %>% pull(pct_step))` and `r label_percent(0.1)(filter(match_flowchart, crit=="c7", vax3_type=="moderna") %>% pull(pct_step))` respectively (Table 1, Figure 1).
...
#### Table 3: Kaplan-Meier 10-week risk estimates

Counts and survival estimates are based on values rounded up to the nearest 7, for disclosure control.
...
contrasts_overall %>%
  transmute(
    ...
    n.atrisk=n.atrisk_0,
    persontime_0=label_number(1)(persontime_0/7), 
    persontime_1=label_number(1)(persontime_1/7),
    n.event_0, 
    n.event_1,
    riskCI_0 = paste0(label_number(0.1, 10000)(1-surv_0), " (", label_number(0.1, 10000)(1-surv.ul_0), "-", label_number(0.1, 10000)(1-surv.ll_0), ")"),
    riskCI_1 = paste0(label_number(0.1, 10000)(1-surv_1), " (", label_number(0.1, 10000)(1-surv.ul_1), "-", label_number(0.1, 10000)(1-surv.ll_1), ")"),
    kmrdCI = paste0(label_number(0.1, 10000)(kmrd), " (", label_number(0.1, 10000)(kmrd.ll), "-", label_number(1, 10000)(kmrd.ul), ")"),
    kmrrCI = paste0(label_number(0.01)(kmrr), " (", label_number(0.01)(kmrr.ll), "-", label_number(0.01)(kmrr.ul), ")"),
    irrCI =  paste0(label_number(0.01)(irr), " (", label_number(0.01)(irr.ll), "-", label_number(0.01)(irr.ul), ")"),
    #coxhrCI =  paste0(label_number(0.01)(kmrr), " (", label_number(0.01)(coxhr.ll), "-", label_number(0.01)(coxhr.ul), ")"),
  ) %>%
  ...
  cols_label(
    ...
    riskCI_0 = "10-week Risk per 10,000 people (95% CI)",
    riskCI_1 = "10-week Risk per 10,000 people (95% CI)",
    
    kmrdCI = "Risk difference per 10,000 people (95% CI)",
    kmrrCI = "Risk ratio (95% CI)",
...
```{r, function-km, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='50%', results='asis', out.extra=''}
...
plot_km_subgroup <- function(subgroup){
    
  subgroup0 <- subgroup
  
  if(subgroup=="all"){
    colour_guide <- list(guides(colour="none"))
  } else{
    colour_guide <- NULL
  }
  ...
  plot_km <- km_estimates %>%
    ...
    ggplot(aes(group=colgroup, colour=subgroup_level_descr, fill=subgroup_level_descr, alpha=treatment_descr)) +
  ...
  plot_kmrd <- contrasts_daily %>%
    filter(
      subgroup==subgroup0,
      outcome %in% outcomes
    ) %>%
    ggplot(aes(group=subgroup_level_descr, colour=subgroup_level_descr, fill=subgroup_level_descr)) +
    geom_step(aes(x=period_start, y=kmrd))+
    geom_rect(aes(xmin=period_start, xmax=period_end, ymin=kmrd.ll, ymax=kmrd.ul), alpha=0.1, colour="transparent")+
    geom_hline(aes(yintercept=0), colour="black")+
  ...
  plot_kmrr <- contrasts_daily %>%
    filter(
      subgroup==subgroup0,
      outcome %in% outcomes
    ) %>%
    ggplot(aes(group=subgroup_level_descr, colour=subgroup_level_descr, fill=subgroup_level_descr)) +
    geom_step(aes(x=period_start, y=kmrr))+
    geom_rect(aes(xmin=period_start, xmax=period_end, ymin=kmrr.ll, ymax=kmrr.ul), alpha=0.1, colour="transparent")+
  ...
  plot_irr <- contrasts_daily %>%
    ...
    ggplot(aes(group=subgroup_level_descr, colour=subgroup_level_descr, fill=subgroup_level_descr)) +
    geom_step(aes(x=period_start, y=irr))+
    geom_rect(aes(xmin=period_start, xmax=period_end, ymin=irr.ll, ymax=irr.ul), alpha=0.1, colour="transparent")+
    geom_hline(aes(yintercept=0), colour="black")+
    facet_grid(rows=vars(outcome_descr), scales="free_y")+
    scale_color_brewer(type="qual", palette="Set1", na.value="grey") +
    scale_fill_brewer(type="qual", palette="Set1", guide="none", na.value="grey") +
  ...
  ggsave(
    filename=glue("km_{subgroup}.png"),
    path=output_dir_rmd,
    plot=plot_combined,
  )
...
#### Figure 3a: Kaplan-Meier cumulative incidence estimates


```{r, figure-km-all, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='50%', results='asis', out.extra=''}
plot_km_subgroup("all")
```
...
#### Figure 3b: Kaplan-Meier cumulative incidence estimates, by primary course type

```{r, figure-km-vax12_type, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='50%', results='asis', out.extra=''}
plot_km_subgroup("vax12_type")
```
...
#### Figure 3c: Kaplan-Meier cumulative incidence estimates, by age group

```{r, figure-km-age65plus, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='50%', results='asis', out.extra=''}
plot_km_subgroup("age65plus")
```
...
#### Figure 3d Kaplan-Meier cumulative incidence estimates, by prior infection status

```{r, figure-km-prior_covid_infection, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='50%', results='asis', out.extra=''}
plot_km_subgroup("prior_covid_infection")
```
...
#### Figure 3e Kaplan-Meier cumulative incidence estimates, by clinical vulnerability

```{r, figure-km-cev_cv, echo=FALSE, message=FALSE, warning=FALSE, out.width='100%', out.height='50%', results='asis', out.extra=''}
plot_km_subgroup("cev_cv")
```
